...
package org.openlmis.integration.dhis2.migration;
...
public abstract class BaseMigrationIntegrationTest {
  ...
  static final String TABLE_WIDGET = "dhis2integration.widget";
  ...
  /**
   * Prepares and puts data into database before the test migrations will be executed. The database
   * will be in version set by {@link #getTargetBeforeTestMigration()}.
   */
  abstract void insertDataBeforeMigration();

  /**
   * Returns to which migration Flyway should migrate before test. If the method returns null as a
   * value, the database will contain a initial schema and bootstrap data.
   */
  abstract String getTargetBeforeTestMigration();

  /**
   * Returns a test migration. if the method returns null as a value, the database will be migrated
  ...
  private String createSql(String template, String... params) {
    return String.format(ENGLISH, template, params);
  }
