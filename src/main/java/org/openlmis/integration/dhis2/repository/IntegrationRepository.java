...
public interface IntegrationRepository
    extends BaseAuditableRepository<Integration, UUID>,
    PagingAndSortingRepository<Integration, UUID> {

  @Query(value = "SELECT"
      + "    i.*"
      + "  FROM"
      + "    dhis2integration.integrations i"
      + "  WHERE"
      + "    id NOT IN ("
      + "      SELECT"
      + "        id"
      ...
      + "          ON CAST(i.id AS varchar) = SUBSTRING(g.local_id, 2, 36)"
